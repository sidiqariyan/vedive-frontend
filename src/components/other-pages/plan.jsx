...
const Plan = () => {
  ...
  const initializePayment = useCallback((paymentSessionId, orderId) => {
    const checkoutOptions = {
      paymentSessionId,
      returnUrl: `http://ec2-51-21-1-175.eu-north-1.compute.amazonaws.com:3000/plans/payment-status?order_id=${orderId}`,
  ...
  const verifyAndProcessPayment = useCallback(async (orderId) => {
    ...
    try {
      ...
      if (!localStorage.getItem("token")) {
        navigate("/login", {
          state: { redirectTo: `http://ec2-51-21-1-175.eu-north-1.compute.amazonaws.com:3000/plans/payment-status?order_id=${orderId}` },
        });
        return;
      }
